...
import COM.RBNELITE.ENTITY.ProductImageDao;
import java.io.File;
...
public class UploadProductImage extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       // processRequest(request, response);
          ...
          try {
            ...
            while (iter.hasNext()) {
                ...
                if (!item.isFormField()) {
                    ...
                    //System.out.println(uploadPath+" :uplllll");
                    File storeFile = new File(filePath);
                    item.write(storeFile);
                    String dataFile=request.getParameter(filePath);
                    int p_idd=(Integer)ses.getAttribute("p_idd1");
                     //String status=userName+" added new photo";
                    
                     try{
                                SessionFactory sf=new Configuration().configure().buildSessionFactory();
                                Session s=sf.openSession();
                                ProductImageDao prodao=new ProductImageDao();
                                prodao.setProd_id(p_idd);
                                prodao.setProd_paath(filePath);
                                s.beginTransaction();
                                s.save(prodao);
        ...
        getServletContext().getRequestDispatcher("/Photo1.jsp").forward(request, response);
