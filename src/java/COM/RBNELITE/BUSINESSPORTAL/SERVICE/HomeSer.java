...
public class HomeSer {
    ...
    public int getNotifications(String user_mail)
    {
        ...
        try
        {
            ...
            s.beginTransaction();  
            List allUsers;  
            Query queryResult = s.createQuery("from NotificationDao where user_mail='"+user_mail+"'");  
    ...
    public List getEvent(String user_mail)
    {
        ...
        SQLQuery queryResult1 = s1.createSQLQuery("SELECT CONCAT (r.fname,' ',r.lname,' created an event : ',e.name) FROM registration r , tbl_event e WHERE e.user_mail=r.email_id AND e.user_mail='"+user_mail+"'");  
   ...
   public List getPeople(String userName)
   {
       ...
       Long sid;
       ArrayList list=new ArrayList();
       SessionFactory sf8 =  new Configuration().configure().buildSessionFactory(); 
       Session s8=sf8.openSession();
       s8.beginTransaction();  
        //SQLQuery queryResult8 = s8.createSQLQuery("SELECT CONCAT(fname,' ',lname) fullname FROM registration WHERE email_id IN(SELECT DISTINCT user_mail FROM tbl_groups WHERE user_mail!='anil@gmail.com' AND group_name IN(SELECT group_name FROM tbl_groups WHERE user_mail='anil@gmail.com')) AND email_id NOT IN(SELECT To_Id FROM tbl_frd_request WHERE From_Id='anil@gmail.com')");  
 SQLQuery queryResult8 = s8.createSQLQuery("SELECT CONCAT(fname,'"+" "+"',lname,'"+":"+"',email_id,'"+"::"+"') fullname FROM registration WHERE email_id IN(SELECT DISTINCT user_mail FROM tbl_groups WHERE user_mail!='"+userName+"' AND group_name IN(SELECT group_name FROM tbl_groups WHERE user_mail='"+userName+"')) AND email_id NOT IN(SELECT To_Id FROM tbl_frd_request WHERE From_Id='"+userName+"') ");
